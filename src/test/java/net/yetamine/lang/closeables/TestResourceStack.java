...
/**
 * Tests {@link ResourceStack}.
 */
public final class TestResourceStack {
    ...
    public void testAdopted_Closeable() throws Exception {
        ...
        try (ResourceGroup<IOException> group = new ResourceStack<>()) {
    ...
    public void testManaged() throws Exception {
        ...
        try (ResourceGroup<IOException> group = new ResourceStack<>()) {
            ...
            final ResourceHandle<AtomicInteger, IOException> handle4 = group.managed(opening, closing);

            resource5 = group.managed(opening, closing).acquired();

            handle2.release();
            Assert.assertEquals(resource2.get(), CLOSED);
            Assert.assertEquals(resource3.get(), CLOSED);
            Assert.assertEquals(resource5.get(), CLOSED);

            resource4 = handle4.acquired();
            resource2.set(INIT);
            resource5.set(INIT);
            ...
            Assert.assertEquals(resource5.get(), INIT);
            resource3.set(INIT);
            resource4.set(INIT);
        }
        ...
        Assert.assertEquals(resource2.get(), INIT);
        Assert.assertEquals(resource3.get(), INIT);
        Assert.assertEquals(resource4.get(), INIT);
        Assert.assertEquals(resource5.get(), INIT);
    }
    ...
    public void testAfterClose() throws Exception {
        ...
        final ResourceGroup<IOException> group = new ResourceStack<>();
