...
public interface MappingStorage<K, V> {
     ...
     */
    static <K, V> MappingStorage<K, V> adapt(Map<K, V> map) {
        return new MapAsTable<>(map);
    }
 ...
 */
enum EmptyTable implements MappingStorage<Object, Object> {
    ...
    public static <K, V> MappingStorage<K, V> getInstance() {
        return (MappingStorage<K, V>) INSTANCE;
    }
...
/**
 * The default implementation for {@link MappingStorage#adapt(Map)}.
 *
 * @param <K>
 *            the type of keys
 * @param <V>
 *            the type of values
 */
final class MapAsTable<K, V> implements Serializable, MappingStorage<K, V> {
     ...
     */
    public MapAsTable(Map<K, V> storage) {
